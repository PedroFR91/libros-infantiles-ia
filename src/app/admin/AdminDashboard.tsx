...
export default function AdminDashboard({ users, books, recentPayments, stats }: AdminDashboardProps) {
          ...
          <div className="bg-surface border border-border rounded-xl p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold">{stats.totalCreditsInSystem}</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-bg">
                  <tr>
                    <th className="text-left px-4 py-3 text-sm font-medium text-text-muted">Usuario</th>
                    <th className="text-center px-4 py-3 text-sm font-medium text-text-muted">Tipo</th>
                    <th className="text-center px-4 py-3 text-sm font-medium text-text-muted">Créditos</th>
                    <th className="text-center px-4 py-3 text-sm font-medium text-text-muted">Libros</th>
                    <th className="text-center px-4 py-3 text-sm font-medium text-text-muted">Pagos</th>
                    <th className="text-center px-4 py-3 text-sm font-medium text-text-muted">Rol</th>
                    <th className="text-center px-4 py-3 text-sm font-medium text-text-muted">Creado</th>
                ...
                <tbody className="divide-y divide-border">
                  {filteredUsers.map((user) => (
                    <tr key={user.id} className="hover:bg-bg/50">
                      <td className="px-4 py-3">
                        <div>
                          ...
                          <p className="text-xs text-text-muted">
                            {user.email || `Session: ${user.sessionId?.slice(0, 8)}...`}
                          </p>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3 text-center">{user._count.books}</td>
                      <td className="px-4 py-3 text-center">{user._count.payments}</td>
                      <td className="px-4 py-3 text-center">
                        {user.role === "ADMIN" ? (
                          <span className="inline-flex items-center gap-1 px-2 py-1 bg-amber-500/20 text-amber-500 text-xs font-medium rounded">
                        ...
                        <div className="flex items-center justify-end gap-2">
                          ...
                          {selectedUser === user.id ? (
                            <div className="flex items-center gap-1">
                              ...
                              />
                              <button
                                onClick={() => handleAddCredits(user.id, creditAmount)}
                                disabled={isUpdating}
          ...
          {activeTab === "books" && (
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-bg">
                  <tr>
                    <th className="text-left px-4 py-3 text-sm font-medium text-text-muted">Libro</th>
                    <th className="text-left px-4 py-3 text-sm font-medium text-text-muted">Usuario</th>
                    <th className="text-center px-4 py-3 text-sm font-medium text-text-muted">Páginas</th>
                    <th className="text-center px-4 py-3 text-sm font-medium text-text-muted">Estado</th>
                    <th className="text-center px-4 py-3 text-sm font-medium text-text-muted">Creado</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border">
                  {filteredBooks.map((book) => (
                    <tr key={book.id} className="hover:bg-bg/50">
                      <td className="px-4 py-3">
                        <div>
                          <p className="font-medium">{book.title || `Libro de ${book.kidName}`}</p>
                          <p className="text-xs text-text-muted">
                      ...
                      <td className="px-4 py-3">
                        <p className="text-sm">{book.user.name || book.user.email || "Anónimo"}</p>
                        <p className="text-xs text-text-muted">{book.user.email}</p>
                      </td>
                      <td className="px-4 py-3 text-center">
          ...
          {activeTab === "payments" && (
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-bg">
                  <tr>
                    <th className="text-left px-4 py-3 text-sm font-medium text-text-muted">ID</th>
                    <th className="text-left px-4 py-3 text-sm font-medium text-text-muted">Usuario</th>
                    <th className="text-center px-4 py-3 text-sm font-medium text-text-muted">Cantidad</th>
                    <th className="text-center px-4 py-3 text-sm font-medium text-text-muted">Créditos</th>
                    <th className="text-center px-4 py-3 text-sm font-medium text-text-muted">Estado</th>
                    <th className="text-center px-4 py-3 text-sm font-medium text-text-muted">Fecha</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border">
                  {filteredPayments.map((payment) => (
                    <tr key={payment.id} className="hover:bg-bg/50">
                      <td className="px-4 py-3">
                        <p className="text-sm font-mono">{payment.id.slice(0, 12)}...</p>
                      </td>
                      <td className="px-4 py-3">
                        <p className="text-sm">{payment.user.name || payment.user.email || "Anónimo"}</p>
          ...
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold">
                    {userDetailModal.name || userDetailModal.email || "Usuario anónimo"}
                  </h2>
                  <p className="text-sm text-text-muted">{userDetailModal.email || userDetailModal.id}</p>
                </div>
              </div>
              <button
                onClick={() => setUserDetailModal(null)}
            ...
            <div className="grid grid-cols-4 gap-4 mb-6">
              <div className="bg-surface rounded-lg p-4 text-center">
                <p className="text-2xl font-bold text-primary">{userDetailModal.credits}</p>
                <p className="text-xs text-text-muted">Créditos</p>
              </div>
              <div className="bg-surface rounded-lg p-4 text-center">
                <p className="text-2xl font-bold">{userDetailModal._count.books}</p>
                <p className="text-xs text-text-muted">Libros</p>
              </div>
              <div className="bg-surface rounded-lg p-4 text-center">
                <p className="text-2xl font-bold">{userDetailModal._count.payments}</p>
                <p className="text-xs text-text-muted">Pagos</p>
              </div>
              <div className="bg-surface rounded-lg p-4 text-center">
                ...
                <p className="text-xs text-text-muted">{userDetailModal.role}</p>
              </div>
            </div>
            ...
            <div className="bg-surface rounded-lg p-4 mb-6">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p className="font-mono text-xs">{userDetailModal.sessionId || "N/A"}</p>
                </div>
                <div>
              ...
              {getUserBooks(userDetailModal.id).length > 0 ? (
                <div className="space-y-2">
                  {getUserBooks(userDetailModal.id).map((book) => (
                    <div key={book.id} className="bg-surface rounded-lg p-3 flex items-center justify-between">
                      <div>
                        <p className="font-medium">{book.title || `Libro de ${book.kidName}`}</p>
                        <p className="text-xs text-text-muted">{book.theme.slice(0, 50)}...</p>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-xs text-text-muted">{book._count.pages} páginas</span>
                ...
                <p className="text-text-muted text-sm">No ha creado ningún libro</p>
              )}
            </div>
              ...
              {getUserPayments(userDetailModal.id).length > 0 ? (
                <div className="space-y-2">
                  {getUserPayments(userDetailModal.id).map((payment) => (
                    <div key={payment.id} className="bg-surface rounded-lg p-3 flex items-center justify-between">
                      <div>
                        <p className="font-medium">€{(payment.amount / 100).toFixed(2)}</p>
                        <p className="text-xs text-text-muted">{formatDate(payment.createdAt)}</p>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-primary font-semibold">+{payment.creditsGranted} créditos</span>
                ...
                <p className="text-text-muted text-sm">No ha realizado ningún pago</p>
              )}
            </div>
            ...
            <div className="mt-6 pt-6 border-t border-border flex gap-3">
              ...
              >
                {userDetailModal.role === "ADMIN" ? "Quitar Admin" : "Hacer Admin"}
              </button>
            </div>
          </div>
        </div>
